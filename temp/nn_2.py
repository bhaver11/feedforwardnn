...
#             print('partial_w',len(partial_w), partial_w[0].shape)
...
def main():
    ...
    train_target = train_target-1922
    model =Normalizer()
    
    train_input = np.array(pd.DataFrame(model.fit_transform(train_input)))
    dev_input = np.array(pd.DataFrame(model.fit_transform(dev_input)))
...
#     print(dev_input)
    ...
    predictions = get_test_data_predictions(net, test_input) + 1922
    pred_idx = np.insert(predictions, 0, range(1,predictions.size+1), axis=1)
    print(predictions)
    np.savetxt('203050058_final.csv', pred_idx, delimiter=',', header='Id,Predicted',fmt='%0.1f,%d',comments="")
